...
- Image generation took **~25s** and returned a **2.3 MB** PNG in live testing.
  `FileUploadType::CoverPhoto` caps at 5 MB, so there is roughly 2× headroom; an oversized image
  throws a `RuntimeException` the controller catches and flashes, rather than failing silently.
- Both generation calls are synchronous inside the admin request. Acceptable for a maintainer
  action, but it is the reason `services.openai.image_timeout` defaults to 180s separately from
  the 90s text timeout.
