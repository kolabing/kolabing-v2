...
| ~~**Free-kolab-used check**~~ | ~~`Profile::hasUsedFreeKolab()`~~ **REMOVED (2026-07-20)** | Businesses no longer get a free kolab, so the freemium quota concept is gone. The publish gate is now a pure subscription check; the onboarding auto-offer bypasses it explicitly (see §3). |
...
|---|---|---|---|
| Create opportunity (legacy shim path) | `OpportunityService::hasReachedFreemiumCollabLimit()` → `!$profile->isBusiness() ? false : !hasActiveSubscription()` | Any unsubscribed business (no free slot) | ✅ correct — **no free collaboration (2026-07-20):** an unsubscribed business is always at the limit; lives only on the legacy `/opportunities` path (port to `/kolabs` tracked in #31) |
...
| Publish kolab (System B) | `KolabService::publish()` — `!$allowUnsubscribedAutoOffer && $creator->isBusiness() && intent_type !== CommunitySeeking && !hasActiveSubscription()` | Business only (auto-offer exempt) | ✅ correct — **no free kolab (2026-07-20):** `hasUsedFreeKolab()` dropped; the onboarding auto-offer passes `allowUnsubscribedAutoOffer: true` (never settable from a controller), every other business promo publish needs a sub |
...
**Identity lock — enforced server-side (2026-07-20). Resolves the golden-rule 4 & 5 violation.**
- Server (feed): `DiscoveryOpportunityResource::identityLockedForViewer()` returns true when a non-subscribed business views a community-authored post. When locked, `creator_profile.display_name` + `avatar_url` are serialized as **null**, `creator_profile.identity_locked = true`, and `resolveCoverPhotoUrl()` no longer falls back to the community avatar (so the logo cannot leak as the cover image). Subscribed businesses / communities see the full identity (`identity_locked = false`).
- Server (profile endpoints): `ProfileController::denyCommunityProfileToUnsubscribedBusiness()` returns **403** (`code: subscription_required`) from `publicProfile` (`GET /profiles/{id}`) and `communityPublicProfile` (`GET /communities/{id}/public-profile`) when a non-subscribed business targets a community. Businesses viewing businesses, communities viewing anyone, and subscribed businesses are unaffected.
- Client: `explore_screen.dart` already computes `hideCreatorIdentity` and blurs via `BlurredIdentity`, and disables the view-profile navigation for a free business. That blur is now a presentation of an already-stripped payload — the security boundary is the server. **No hard block on Explore.**

**Enforcement principle:** the identity is withheld by the server (mechanism), not merely blurred by the client (convention). Reveal-on-subscribe works via a re-fetch (the client already refreshes on subscription-state change). Tests: `DiscoveryOpportunityControllerTest` (masked vs revealed), `CommunityProfileVisibilityTest` (403 gates).
...
- [x] `KolabService::publish` gated by `isBusiness()`. **No free kolab (2026-07-20):** the gate no longer depends on `hasUsedFreeKolab()` (method removed) — every self-initiated business promo publish requires an active subscription. The one onboarding auto-offer bypasses via `publish(..., allowUnsubscribedAutoOffer: true)`, a flag no controller sets. Legacy `OpportunityService::hasReachedFreemiumCollabLimit()` likewise returns true for any unsubscribed business (no count-based free slot). (§3)
- [x] **Community identity withheld from free businesses (2026-07-20):** discovery feed nulls `creator_profile.display_name`/`avatar_url` + emits `identity_locked` (and never leaks the community logo via the cover-photo fallback) for a non-subscribed business viewing a community; `GET /profiles/{id}` and `GET /communities/{id}/public-profile` return 403 (`subscription_required`) in the same case. (§4)
...
- [x] **Blur/withhold (name + logo) for free businesses on Explore — DONE (2026-07-20).** Server nulls the community identity and emits `creator_profile.identity_locked = true` for a non-subscribed business viewing a community post; the client renders its locked state over the already-stripped payload. **No hard block on Explore** — the Kolab card and all details remain. (§4)
