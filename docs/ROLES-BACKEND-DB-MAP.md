...
## 19. Kolabs on the open web — backend map (added 2026-08-22)
...
associative-boolean shape, so a test built on the factory renders empty labels rather than failing — BACKLOG **BE-FX-22**.
...
## 20. Explore as an agenda + the Kolab drawer (added 2026-08-22)
...
map `KolabFactory` still writes (BE-FX-22), so a factory-built fixture renders instead of
silently emptying.
...
account. Draft → the in-app URL, because the public page 404s by design (§19 / ROLES §4.3).
