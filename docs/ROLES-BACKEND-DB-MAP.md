...
| Accept application | none — `ApplicationPolicy::accept()` / `ApplicationService::validateCanAccept()` no longer check subscription | nobody | ✅ correct (fixed 2026-09-25, BE-FX-73). Accepting was never one of the two §2.7 paywalled actions; the code had drifted and gated it anyway. |
...
The two remaining backend gates (apply, publish) follow the same pattern: `if ($profile->isBusiness() && ! $profile->hasActiveSubscription())`. **Never copy this gate into community or attendee paths, and never reintroduce it on accept.**
...
Accepting an application no longer has a paywall status to reconcile (BE-FX-73, 2026-09-25) — a business without a subscription accepts like any other. The `?reason=accept` client redirect (below) is now dead code on the server side: nothing returns 402/403 for accept, so it never fires.
...
- [x] **BE-FX-73 — FIXED 2026-09-25. Accepting an application was still subscription-gated, contradicting §2.7's "exactly two actions."** `ApplicationPolicy::accept()` and `ApplicationService::validateCanAccept()` both required `hasActiveSubscription()` for a business creator; removed. Found during the 25 Sep Kolabing team WhatsApp code check; Daniel confirmed accepting stays free. See §3.

Still open:

- [ ] **Venue Pro insights must stay the only Pro-gated surface** (§36, BE-NF-68). Calendar, own events, attendance, revenue logging and attendee messages are free for every business; a PR that puts any of them behind `hasVenueProAccess()` contradicts ROLES §2.19. The insights endpoint itself is not built yet.
- [ ] **Revenue must never reach the community side or attendees** (§36). Enforce when the results columns land on `events` (spec §9): `Event::$hidden` plus a test on every attendee- and community-facing event response.
...
- [x] **BE-FX-68 — FIXED 2026-09-25 (PR open). The "new application" notification named the community to a free business** — text, push, email, actor fields and the linked application summary; `public_channels` also leaked under the BE-FX-22 mask. See §26.5.
...
### 26.5 Notifications and the application summary (BE-FX-68, added 2026-09-25)

The mask had one more door than §26.1 listed: the notification.

| Surface | Where | Masked shape |
|---|---|---|
| Notification text (in-app row, push, email merge vars) | `NotificationService::nameForRecipient()` — `application_received`, `application_withdrawn` (creator copy), every `notifyBothParties()` `:name` and `partner_name`, `collab_followup_reminder` `partner_name` | `notifications.masked_community` in the recipient's locale. Written into the row, so it stays masked after a later subscribe. |
| `GET /me/notifications` actor | `NotificationResource` | `actor_profile_id`, `actor_name`, `actor_avatar_url` null + `actor_identity_masked: true`. Decided at read time, so subscribing reveals it. The id is withheld too (same reasoning as `SuggestionResource`). |
| `applicant_profile` on an application | `ApplicationResource` → `ProfileSummaryResource::maskIdentity()` | `display_name`, `avatar_url` null, `portfolio_photos` and `public_channels` empty, `identity_masked: true`. `id` kept (the app addresses it). `identity_masked` is only emitted where the mask is evaluated. |

`public_channels` (inside the verification fields) carries the community's Instagram / TikTok /
website URLs. It was missing from §26.3's withheld list, so `CommunityPublicProfileResource` still
returned it under the mask. Now emptied there and in the summary.

**Not masked, deliberately:** the multi-Kolab organizer notifications (the organizer runs the
event and is gated by the organizer entitlement, not the business paywall — a product call), chat
message bodies (user text; chat exists only after an accepted application), and the application
`message` / `availability` free text (a community could sign its name there; not filterable).
`ProfileSummaryResource` elsewhere (Explore / Kolab creator) is still unmasked — that is BE-FX-21.
...
## 33. Admin full onboarding — the app's own wizard, run by a maintainer (BE-NF-64, added 2026-09-22)

Extends §29 (quick-add). Quick-add **lists** a profile; this **onboards** one. See ROLES §2.17 for
the *what*; this is the wiring.

| Route | Controller | Guard | Notes |
|---|---|---|---|
| `GET /admin/users/onboard?type=business\|community` | `Admin\ManagedUserController::onboardForm` | `auth:admin` + `maintainer` | One view, two forms. Declared **before** the `{profile}` routes so `onboard` is never captured as a profile id. |
| `POST /admin/users/onboard/business` | `…::onboardBusiness` | same | `Admin\AdminBusinessOnboardingRequest` |
| `POST /admin/users/onboard/community` | `…::onboardCommunity` | same | `Admin\AdminCommunityOnboardingRequest` |

**The route decides the user type, not the payload.** Both controller methods append
`user_type` themselves (`[...$request->validated(), 'user_type' => UserType::Business->value]`), so a
tampered form field cannot make the business form create a community.

### One onboarding implementation, two entry points

`ManagedProfileService::onboard()` creates the shell account (random 32-char password, never shown;
`email_verified_at = now()`; the detail row via the existing `upsertDetailProfile()`, which must exist
first because `OnboardingService` **updates** `businessProfile`/`communityProfile` and would fatal on a
null relation) and then hands straight to:

- `OnboardingService::completeBusinessOnboarding($profile, $data)`, or
- `OnboardingService::completeCommunityOnboarding($profile, $data)`

— the same methods `Api\V1\OnboardingController` calls for
`PUT /api/v1/onboarding/{business,community}`, with the same validated array. `ManagedProfileService`
gained a constructor for the injected `OnboardingService`; it had none before.

Everything downstream therefore happens for free and is *not* re-implemented here: the business
auto-offer (`provisionBusinessAutoOffer()` — still exactly one, still idempotent), the completion
missions, `GenerateSuggestionsForProfile`, the drip enrolment, and
`Profile::onboardingCompleted()` flipping true.

**Failure handling:** `completeBusinessOnboarding()` opens its own transaction and dispatches after
commit, so it runs *outside* the shell's transaction. If it throws, `discardShellProfile()`
`forceDelete()`s the shell — otherwise a half-built account would hold the email address hostage
against the retry (the unique rule is `Rule::unique('profiles','email')->whereNull('deleted_at')`, so a
soft delete would not free it either). The cleanup is best-effort and logs rather than masking the real
exception.

### The request classes extend, never restate

`AdminBusinessOnboardingRequest extends Api\V1\BusinessOnboardingRequest`, ditto community. Only
`email` is added — the app already has an authenticated profile by the time it reaches onboarding. A
rule change in the app's onboarding applies to the admin form automatically, which is the whole point:
quick-add validated its own thinner shape and that is why it produced profiles the app's own onboarding
would have rejected.

Two overrides are structural, not incidental:

- **`failedValidation(): never`** — the parent throws a 422 JSON body, which on an HTML form renders as
  a blank page with the maintainer's typing gone. Replaced with Laravel's default redirect-back.
  The signature must stay `never`: PHP return types are covariant and `never` cannot be widened.
- **`prepareForValidation()`** (business only) reassembles the app's payload shape out of what an HTML
  form can post, then calls `parent::prepareForValidation()`:
  1. `primary_venue` and `opening_hours` arrive as **JSON strings** — the shared
     `admin.users._places-import` partial writes the whole Google Places payload into one hidden input.
  2. `venue[...]` holds what a human must confirm. `primary_venue.venue_type` and
     `primary_venue.capacity` are `required_with:primary_venue` and **Google publishes neither**. They
     are overlaid *on top of* the import, so a maintainer's correction always wins.
  3. Places returns venue photos as objects (`{resource_name: …}`) while the rules validate
     `primary_venue.photos.*` as strings. Flattened here so the API's rules stay untouched.
  4. `has_venue = false` nulls `primary_venue` outright — an empty array would still trip
     `required_with` on venue_type/capacity.

### What is *not* wired

No password is set (the owner creates one via §30's manual, language-picked, preview-gated welcome
email — `onboard()` sends nothing). `verification_channels` stays optional and is not on the form.
Attendee is rejected with `InvalidArgumentException`: there is no attendee route, and
`completeAttendeeOnboarding()` needs an identity (handle, interests) a maintainer cannot invent.

**No gate moved.** No paywall, capability, or role condition is touched — a community still gets no
subscription row, and the business free tier is still exactly one published Kolab.

**Tested:** `tests/Feature/Admin/AdminFullOnboardingTest.php` — 12 tests: maintainer-only, both forms
render, the business gets `categories`/`has_venue`/`venue_type`/`capacity`, `onboardingCompleted()` is
true, exactly one auto-offer exists, a no-venue business needs no venue details, >3 categories is
rejected by the *app's* rule, the community gets the `community_type` quick-add could not set, no
subscription row is created for a community, nothing is mailed, and a taken email creates no partial
account.

---

## 34. Admin sales mailing — generated outreach (BE-NF-65, added 2026-09-22)

Extends §9 (admin operator surfaces) and §33 (full onboarding, which produces the accounts this
pitches to). ROLES §2.18 for the *what*.

| Route | Controller | Guard |
|---|---|---|
| `GET /admin/sales-mailing` | `Admin\SalesMailingController::index` | `auth:admin` + `maintainer` |
| `POST /admin/sales-mailing` | `::generate` | same |
| `GET /admin/sales-mailing/{draft}` | `::edit` | same |
| `POST /admin/sales-mailing/{draft}/idea` | `::selectIdea` | same |
| `POST /admin/sales-mailing/{draft}/image` | `::generateImage` | same |
| `PUT /admin/sales-mailing/{draft}` | `::updateCopy` | same |
| `GET /admin/sales-mailing/{draft}/preview` | `::preview` | same |
| `POST /admin/sales-mailing/{draft}/send` | `::send` | same |

### Schema

`sales_outreach_drafts` — one pitch, from generation to send:

| Column | Type | Note |
|---|---|---|
| `id` | uuid | |
| `business_profile_id` | uuid FK → `profiles` | recipient; cascade |
| `community_profile_id` | uuid FK → `profiles` | the pitched partner; cascade |
| `locale` | varchar(5) | `en` / `es` (`config/sales_outreach.locales`) |
| `kolab_ideas` | jsonb | **every** generated alternative, not just the chosen one |
| `selected_idea_index` | smallint | |
| `cover_image_url` | varchar(2048) | nullable |
| `expected_attendees`, `avg_spend_cents`, `estimated_revenue_cents` | int / bigint | the inputs **and** the result |
| `subject`, `body_markdown` | varchar / text | generated, then maintainer-editable |
| `status`, `sent_at` | varchar(16) / timestamp | `draft` \| `sent` |
| `created_by` | **bigint** FK → `users` | nullOnDelete. `users.id` is a bigint, unlike every profile-side uuid here |

Storing the estimate's *inputs* alongside its result is deliberate: keeping only the total would
make an old pitch unexplainable the moment the default assumptions change, and this is a number a
business was invited to check.

### The OpenAI integration

`App\Services\OpenAi\OpenAiClient` — the only place that talks to OpenAI, over Laravel's HTTP
client. **No composer dependency was added.** Config in `config/services.php` under `openai`;
model ids were verified against a live `GET /v1/models` for the project key rather than guessed
(`gpt-5.2` for text, `gpt-image-2` for images). A missing key throws rather than degrading to an
empty pitch, and `isConfigured()` lets the form disable its own button with a reason.

`Http::fake()` is therefore the whole testing story — no provider swapping, no interface ceremony.

### Who computes what

- `SalesPitchGenerator` — two calls, not one: `generateIdeas()` then `composeEmail()`. Asking for
  three complete emails up front would triple the cost of the common case. Its prompt is built
  only from stored profile fields, with absent fields **omitted** rather than filled with
  placeholders, so the prompt never implies a fact the database does not hold.
- `RevenueEstimator` — plain PHP. `attendees × avg_spend`, with turnout suggested as
  `community_size × attendance_rate` clamped between `min_attendees` and `max_attendees`
  (`config/sales_outreach.php`). A community with no stated size falls back to the floor, not to
  zero: an unfilled field is not an empty community. **The model never sees or produces this
  number** — it is handed the already-formatted strings and told not to recalculate.
- `SalesOutreachService` — orchestrates, and owns the ordering rule that nothing sends without a
  human. Switching idea **regenerates** the copy and **clears** the cover rather than patching
  either: an email written for a wine tasting does not become one for a run club by swapping a
  title, and the old cover illustrates the old idea.

### Delivery

`SalesPitchMail` (`ShouldQueue`), markdown view `mail/sales-pitch.blade.php`, fixed strings in
`lang/{en,es}/sales_mail.php`. `Mail::locale()` is set in the constructor from the draft — without
it the body is Spanish and the button under it is English, the same bug §30 hit on the welcome email.

Sent with a plain `Mail::to()->queue()`, **not** `EmailService` — see ROLES §2.18 for why
(`marketing_tips` defaults to false, so every pitch would be silently suppressed).

The generated copy is the only editable half; the greeting, estimate panel, CTA, its URL and the
sign-off are the template's. Same security boundary as §30's welcome email, and load-bearing here
because the editable half was written by a language model.

### Cover generation is asynchronous (BE-FX-61)

It was not, at first, and production answered a **Cloudflare 504** on
`POST /admin/sales-mailing/{draft}/image`. Image generation measured ~25s on its own in
testing and the R2 upload follows it; Cloudflare gives up at 100s and Laravel Cloud has its own
ceiling. Raising a timeout would only have moved the failure — a request that waits on a
third-party image model is a request that eventually exceeds somebody's limit.

- `queueCoverImage()` validates cheaply (idea present, prompt present, key configured — so an
  impossible request still fails on the maintainer's screen), marks the draft `pending` **before**
  dispatch (a fast worker would otherwise have its `ready` overwritten), and returns.
- `GenerateSalesPitchCoverImage` (`tries = 1`, `timeout = 300`) calls `drawCoverImage()` on a
  worker. One try on purpose: every attempt is billed, and the realistic failures — a
  content-policy refusal, a bad key — are not transient. `OpenAiClient` already retries once at
  the HTTP layer for a genuine 429/5xx.
- New columns `cover_image_status` (`idle` | `pending` | `ready` | `failed`) and
  `cover_image_error`. The failure path writes both from `drawCoverImage()` *and* from the job's
  `failed()` hook, because the maintainer is no longer watching the response that does the work —
  a failure that only reaches the log is experienced as a spinner that never resolves.
- The edit page reloads itself every 6s while `pending`, and stops because the block that emits
  the script stops rendering. Deliberately not a polling endpoint: one maintainer has one of these
  open at a time and the job finishes well inside a minute.
- Switching idea resets the status to `idle` alongside clearing the URL.

### The form asks with the app's own choice boxes (BE-NF-66)

The Full Onboarding form (§33) originally used a hardcoded `VenueType` enum in a
`<select>` and a free-text box for offerings. The app does neither: venue type is a row
of tappable chips (`business_step2_screen.dart`) and offerings are multi-select toggle
cards (`offering_screen.dart`), both fed by `/api/v1/lookup/*`, which reads the
admin-managed `offer_options` table. An option a maintainer added in
`/admin/offer-options` therefore appeared on phones and not in the panel.

- `admin.users._choice-chips` is one partial for all three lists (venue type, offerings,
  product type), reading `/api/v1/lookup/{venue-types,offerings,product-types}` — the
  same public endpoints the Maps import card already calls, so no token is involved.
- **`offering` is still stored as free text**, because that is the column and
  `provisionBusinessAutoOffer()` uses it as the auto-offer's *description*. The picked
  slugs are converted to their human labels in
  `AdminBusinessOnboardingRequest::composeOffering()` and the maintainer's own free text
  is appended. Writing slugs there would have published `venue,food_drink,discount` to
  communities as the pitch. Composed server-side on purpose: the label lookup belongs to
  the side that owns the taxonomy.
- The venue **address** is a Places autocomplete (`admin.users._address-autocomplete`)
  against `/api/v1/places/autocomplete`, the same endpoint mobile uses. It needs no
  `/places/details` follow-up — autocomplete already returns `formatted_address`, `city`,
  `city_id` and coordinates on every row — and it fills the city fields only when they
  are empty, the same rule the import card follows.

### The cover image is briefed from data, and grounded in their own photos (BE-NF-66)

The first covers were generic stock-looking rooms, because the brief was the single
sentence the text model invented alongside the idea. `CoverImageBrief` now composes it
from stored data — venue type and capacity, city, the business's categories, what each
side brings to *this* Kolab, the community's type, the expected turnout — with the
model's line kept as the opening creative direction.

It also collects up to **three reference photographs** (the venue photo first, then the
two profile photos) and `OpenAiClient::imageWithReferences()` posts them to
`/images/edits` instead of `/images/generations`, so the generated room takes its
palette and materials from the real place. No references → plain generation. A
reference that will not download is skipped, not fatal: a cover grounded in two photos
is still a cover, and one slow CDN must not fail a pitch.

**Logos are references, never subjects.** The brief forbids text and logos in the output
because generative models render lettering as convincing gibberish, and a mangled
version of a business's own mark in a cold sales email is worse than no mark at all. If
the real logos are wanted *visibly*, that is a compositing step over the finished
render, not something to ask the model for.

### Prospect research, the chosen angle, and WhatsApp (BE-NF-67)

`ProspectIntel::gather()` researches the business before the email is written. Three
sources, and the largest one costs nothing:

1. **What we already stored.** `business_profiles.primary_venue` has carried the
   Google `rating`, `user_ratings_total`, `opening_hours`, `price_level` and
   `google_place_types` since import and was used for nothing. 7 of 8 production
   businesses have it. This alone is the biggest lift in the pitch's specificity.
2. **Their own website** (`sales_outreach.intel.website`) — title, meta description
   and a 1 500-character text excerpt, scripts and styles stripped.
3. **Weather** (`sales_outreach.intel.weather`) — Open-Meteo, no key, 7-day forecast
   at the venue's coordinates.

Each source is independently switchable because each fails differently, and every
failure degrades the brief rather than the pitch: an unreachable site or a dead
weather API leaves the other sources intact.

**The website fetch is SSRF-guarded.** `website` is user-supplied and we make our own
server fetch it; the `url` validation rule checks the shape, not the destination.
`isFetchable()` requires http/https, resolves the host, and rejects private and
reserved ranges — so `127.0.0.1`, `10.x`, `192.168.x`, `169.254.169.254` (cloud
metadata) and non-http schemes never leave the process. Pinned by a data provider in
`ProspectIntelTest`, which also runs `Http::preventStrayRequests()`: the first draft of
that file used a non-matching fake pattern and silently asserted against the real
example.com.

**The model chooses the angle** (`quiet_hours`, `reviews`, `capacity`, `weather`,
`website`, `generic`) and returns which one it picked; it is stored in
`sales_outreach_drafts.angle` so a maintainer sees the strategy without re-reading the
copy, and so it becomes possible to learn which angles land. The prompt requires the
chosen fact to be used accurately and to fall back to `generic` rather than invent an
observation — the owner knows their own business and will spot a fabricated one.

`intel` is stored on the draft rather than recomputed, because a pitch claiming "Tuesday
morning is your quietest shift" has to stay explainable after their hours change.
Switching idea reuses the stored research instead of re-fetching: it describes the
business, which did not change because a different idea was picked.

**WhatsApp is a link, not an integration.** The same call returns a
`whatsapp_message` (under 60 words, no markdown, no links) and
`SalesOutreachDraft::whatsappLink()` builds a `wa.me` deep link. Deliberately **not**
the WhatsApp Business Platform: that requires pre-approved templates for
business-initiated conversations, and messaging people who never opted in breaches
Meta's Business Messaging Policy and gets numbers banned. A link the maintainer clicks,
which opens their own WhatsApp with the text filled in, is compliant and needs no
integration. Null when there is no message or no number on file.

**Instagram is deliberately absent.** The official Graph API needs the business to
authorise us and scraping breaks Meta's terms, so there is no version that keeps
working. Handles are stored; they are not a data source.

### Writing the pitch is asynchronous too (BE-FX-63)

BE-FX-61 queued the cover and left the copy in the request. The copy is the slower
half — research plus **two `gpt-5.2` calls** — and it kept timing out at Cloudflare.

The diagnosis came from the drafts table rather than the logs: **two complete drafts
a minute apart**, both with copy and a WhatsApp message. The work had never failed;
the response was lost and the maintainer's retry silently created a second record.
Locally the same work measures ~23s, which is why it shipped synchronously; from
production the model calls run three to four times slower.

- `queuePitch()` creates the draft with the revenue arithmetic (instant, and the
  maintainer's own inputs belong to the request that supplied them) and dispatches
  `WriteSalesPitch`. `tries = 1`: a retry would hand the maintainer a second set of
  ideas for the same pair.
- `writePitch()` runs on the worker and records failures on the draft as well as
  throwing, because the page polls and cannot otherwise tell slow from dead.
- `kolab_ideas`, `subject` and `body_markdown` are now **nullable** — a queued draft
  exists before the model has written anything. Changed through the schema builder,
  not raw `ALTER COLUMN … DROP NOT NULL`, which is Postgres-only and breaks the
  SQLite test database.
- `assertEditable()` gained an `isWritten()` guard, so send, edit, switch-idea and
  cover generation all refuse until the copy exists — a cover drawn earlier would
  illustrate an idea that does not exist yet.

### Nothing on this page calls a model inside a request (BE-FX-64)

`selectIdea` was the last one — rewriting the copy for a different idea is another
`gpt-5.2` call, ~4.5s from a laptop and therefore ~20s from production. Under the
edge timeout, but that is precisely the reasoning that put generation in the request
to begin with, and it was wrong twice.

It now dispatches the same `WriteSalesPitch` job with an idea index: `null` means
write from scratch, an integer means rewrite around an already-generated idea using
the **stored** research. Reusing `generation_status` means the page's existing
"writing…" banner, its self-refresh and every copy-dependent guard apply unchanged.

The full inventory of slow work on this surface, all of it now queued: writing the
pitch (research + two text calls), rewriting for an idea (one text call), and drawing
the cover (one image call plus an upload).

### Other operational characteristics

- A generated cover was **2.3 MB**; `FileUploadType::CoverPhoto` caps at 5 MB, roughly 2× headroom.
  An oversized image throws a `RuntimeException` recorded on the draft rather than failing silently.
- The **text** calls are still synchronous inside the admin request. They are far shorter than the
  image call, but they are the same shape of risk, and `services.openai.timeout` (90s) is already
  close to Cloudflare's 100s. If a 504 ever appears on *generate*, this is why, and the fix is the
  one already applied here.

**Tested:** `tests/Feature/Admin/SalesMailingTest.php` — 23 tests, `Http::fake()` keyed on the
prompt rather than call order (a sequence would silently hand the email call the ideas payload).
Covers: maintainer-only, missing-key messaging, the revenue arithmetic being independent of the
model, turnout estimation/cap/floor, both locales, the Spanish furniture and the estimate
disclaimer actually rendering, the pair being business+community, generate-sends-nothing,
preview-sends-nothing, send queues and marks, no double-send, no post-send edit, and both an
OpenAI text failure and an image failure leaving nothing behind.

**Verified live** against the real API on the `development` database: a Spanish pitch for a real
business/community pair produced three distinct, on-brief ideas and copy that used the actual
member count and city, printed the estimate as `75 × EUR 18 = EUR 1,350`, and closed with a single
ask. The image endpoint returned a valid PNG; the *upload* then failed locally because the local
...
## 36. Venue Pro — the plan and its entitlement (BE-NF-68, added 2026-09-24)

Step 1 of the spec (`docs/superpowers/specs/2026-09-24-venue-pro-hotel-dashboard-design.md`). The rule is ROLES §2.19.

**Tables/columns:** `business_subscriptions.plan` — `string(20)`, default `'standard'`, values `standard | pro` (migration `2026_09_24_094815_add_plan_to_business_subscriptions_table`). Every pre-existing row is the €49 plan, which is the default, so there is no backfill. `BusinessSubscription::$attributes` mirrors the default so a freshly created row reports its plan without a refresh.

**Enum:** `App\Enums\SubscriptionPlan` (`Standard`, `Pro`). `forCheckoutKey()` maps a checkout key (`monthly`, `three_months`, `pro_monthly`) to a plan through the `plan` key of each `config('subscriptions.business.stripe.*')` entry; `fromStripePriceId()` is the reverse lookup and returns null for an unknown price.

**Entitlement:** `Profile::hasVenueProAccess()` = business && (`is_test_user` || `subscription->isActivePro()`). Pro is a superset, so `hasActiveSubscription()` is unchanged and still true for a Pro business. Nothing calls `hasVenueProAccess()` yet — the insights endpoint is spec step 5.

**Where the plan is written, and from what:**

| Path | Code | Plan source |
|---|---|---|
| Checkout activation (webhook + synchronous confirm) | `SubscriptionService::activateFromStripeSession()` | the subscription item's Price; unknown → `standard` (a lapsed Pro row never leaks into a new purchase) |
| `customer.subscription.updated/deleted` | `SubscriptionService::syncFromStripeSubscription()` | the Price; unknown → keep the stored plan |
| Plan change | `SubscriptionService::changePlan()` → `StripeService::changeSubscriptionPrice()` | Stripe's answer to the item swap |
| App Store purchase | `AppleIAPService::appleIdentityAttributes()` | always `standard` |
| Maintainer grant | `ManagedProfileService::grantSubscription(…, SubscriptionPlan)` via `GrantSubscriptionRequest` | the button pressed; default `standard` |

**Endpoints:** `POST /api/v1/me/subscription/checkout` accepts `plan = pro_monthly`. New `POST /api/v1/me/subscription/change-plan` (`ChangeSubscriptionPlanRequest`, `plan` ∈ the checkout keys): 403 for a community, 409 with no active plan, for `apple_iap` / `maintainer` sources, or when already on the requested price; 502 on a Stripe error. An upgrade to Pro uses `proration_behavior = always_invoice`, anything else `create_prorations`. `GET /me/subscription` gains `plan` and `plan_label`.
