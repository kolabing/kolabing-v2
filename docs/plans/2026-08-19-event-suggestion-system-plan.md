...
- Base query: `Profile::query()->where('user_type', $counterpartType)` scoped to the city set. **Read the city from the extended profile** (`business_profiles.city_id` / `community_profiles.city_id`), falling back to `profiles.city_id` — that column is attendee-only with no backfill, so scoping on it alone matches almost nothing in production. For a business viewer the set widens to `[city_id, ...business_profiles.target_city_ids]`; widening for a business *counterpart* is deliberately not done (it needs a JSON predicate in SQL, and BE-FX-12 argues against engine-specific SQL the SQLite suite cannot verify).
- `whereNotExists` for `user_blocks` in either direction.
- `whereNotExists` for `applications` in a pending state and `collaborations` in `scheduled`/`active` between the pair, matched on **`creator_profile_id` / `applicant_profile_id`**. Do *not* use `business_profile_id` / `community_profile_id`: they are nullable FKs to the *extended* profile tables, so comparing them against `profiles.id` means the exclusion never fires and we keep suggesting pairs that are already working together. `applications` has no counterpart column and is joined through `kolabs`.
...
**Add the pair; do not rename the existing one.** Renaming only moves *which* required field ends up empty — under either reading of the field names, exactly one required field of a `community_seeking` Kolab stays unfillable with a single intersection. And the two intersections are not interchangeable: `offers_in_return` draws on the `KIND_DELIVERABLE` vocabulary while `needs` draws on `KIND_NEED`, so one array cannot serve both slots. With both intersections present, `suggested_format.offer` and `expects` fill from one each and stay audience-independent.

**Production holds two JSON shapes in the offer/need columns — normalise on read.** Verified read-only, 2026-08-19: `kolabs.needs` 8 object / 19 array, `offers_in_return` 22 / 19, `offering` 27 / 26, `expects` 27 / 17. Roughly 45% of rows carry the legacy keyed-boolean shape, e.g. `{"venue":true,"food_drink":false,"discount":{"enabled":false}}`. Validators only accept lists; the factory writes objects; both are in the table. An `array_intersect` against a legacy row yields nothing, so `offer_need_fit` returns a **false 0.0** — which scores the pair as actively bad rather than unknown. Normalise on read: an object becomes the keys whose value is `true` or whose value is an object with `enabled: true` (`discount` nests); an array passes through.

**`has_venue` cannot be derived from capacity.** 62 businesses have `has_venue = true`; only 44 carry a `primary_venue.capacity` key and 44 a `latitude`. The validation chain implies capacity is required, but 18 live rows predate or bypass it, so deriving "is a venue" from `capacity > 0` mislabels them and proposes `product_promotion` where `venue_promotion` belongs. `PairContext` carries explicit `viewerHasVenue` / `counterpartHasVenue`; capacity is used only for the attendance cap.

**Vocabularies come from `kolabs`, not `business_profiles`.** `business_profiles.offering` is a free-text sentence, not slugs. All four arrays read from `kolabs`: `offering`/`expects` for a business, `offers_in_return`/`needs` for a community — which also keeps each intersection vocabulary-consistent.

**Alias offers against needs for comparison only.** `venue`/`venue_space`, `food_drink`/`free_drinks` and `sponsorship`/`sponsor` are the same thing under different slugs, so a plain intersection yields a false `offer_need_none`. Extract Explore's existing `OFFER_TYPE_ALIASES` to `app/Support/Matching/OfferTypeAliases.php` and use it **only** when comparing. The values that land in `suggested_format.offer` must stay in the `KIND_OFFERING` vocabulary the create form validates, so they are never aliased.
...
- `KolabSuggestion::updateOrCreate(['viewer_profile_id','counterpart_profile_id'], [...])` — one row per pair, forever. Every re-run (same day or a month later) updates that row instead of adding a card. `batch_key` and `expires_at` move forward in the payload. **Never put `shown_at`/`clicked_at`/`dismissed_at` in the update payload** — that is what makes the funnel and the dismissal survive a re-score. Clearing `dismissed_at` is **this task's** job, not the finder's: Task 5 only *excludes* pairs dismissed inside the cooldown and never writes, so it cannot clear anything. When you refresh a pair whose `dismissed_at` is older than the cooldown, clear it — otherwise the cooldown is a permanent block. Note this cannot be expressed with `updateOrCreate`, because whether to clear depends on the *stored* value: use `firstOrNew` + `fill` + `save` on the same key.
