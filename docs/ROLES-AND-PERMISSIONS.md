...
`email_verified_at` is set, so the listing behaves like any other from creation) and emails the owner
`AdminProfileWelcomeMail`: a "what happens next" explainer plus a **create-password link**. That link
reuses the standard `Password::broker()` reset-token flow (the same mechanism behind the ordinary
forgot-password screen) rather than a bespoke magic-link system — so a quick-added owner sets their own
password through the exact same page and code path as everyone else.
...
**The city filter matches the city, not the spelling (BE-FX-56, 2026-09-22).** Visibility only — no gate moved, and
...
resolution feeds the recommended feed's city boost and the `city_match` reason. Backend map §30.
