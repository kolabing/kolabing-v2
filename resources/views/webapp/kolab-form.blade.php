...
<div x-data="kolabForm()" x-init="init()">
    ...
    <main class="max-w-xl mx-auto px-5 py-8">
        <h1 class="font-montserrat font-black text-2xl tracking-tight" x-text="isEdit ? t('form.edit_title') : t('form.create_title')"></h1>
        ...
        <template x-if="!loading && canPickVenue">
            <div class="mt-4 rounded-xl bg-off-black/5 text-off-black/70 text-sm px-4 py-3">{{ __('webapp.form.venue_note') }}</div>
        </template>

        <form x-show="!loading" @submit.prevent="submit()" class="mt-5 space-y-4">
            <div>
                <label class="text-sm font-semibold">{{ __('webapp.form.field_title') }}</label>
            ...
            <div>
                <label class="text-sm font-semibold">{{ __('webapp.form.headline') }} <span class="text-off-black/40 font-normal">({{ __('webapp.form.headline_hint') }})</span></label>
            ...
            <div>
                <label class="text-sm font-semibold">{{ __('webapp.form.description') }}</label>
            ...
            <div>
                <label class="text-sm font-semibold">{{ __('webapp.form.city') }}</label>
            ...
            <div>
                <label class="text-sm font-semibold">{{ __('webapp.form.cover') }} <span class="text-off-black/40 font-normal">({{ __('webapp.common.optional') }})</span></label>
                <template x-if="form.cover">
                    <div class="mt-2 relative">
                        <img :src="form.cover" alt="" class="w-full h-40 object-cover rounded-xl">
                        <button type="button" @click="form.cover = ''" class="absolute top-2 right-2 rounded-full bg-off-black/70 text-off-white text-xs font-semibold px-2 py-1">{{ __('webapp.common.remove') }}</button>
                    </div>
                </template>
                <template x-if="!form.cover">
                    <input type="file" accept="image/*" @change="onCover($event)" class="mt-1 block w-full text-sm text-off-black/70">
                </template>
                <p class="text-xs text-off-black/50 mt-1" x-show="uploading">{{ __('webapp.common.uploading') }}</p>
            </div>
            ...
            <template x-if="form.intent_type === 'community_seeking'">
                <div class="space-y-4">
                    <div>
                        <label class="text-sm font-semibold">{{ __('webapp.form.needs') }}</label>
                        <div class="mt-2 flex flex-wrap gap-2" x-html="chips('needs', lookups.needs)"></div>
                    </div>
                    <div>
                        <label class="text-sm font-semibold">{{ __('webapp.form.typical_attendance') }}</label>
                    ...
                    <div>
                        <label class="text-sm font-semibold">{{ __('webapp.form.offers_in_return') }}</label>
            ...
            <template x-if="form.intent_type === 'product_promotion'">
                <div class="space-y-4">
                    <div>
                        <label class="text-sm font-semibold">{{ __('webapp.form.product_name') }}</label>
                    ...
                    <div>
                        <label class="text-sm font-semibold">{{ __('webapp.form.product_type') }}</label>
                    ...
                    <div>
                        <label class="text-sm font-semibold">{{ __('webapp.form.offering') }}</label>
                        <div class="mt-2 flex flex-wrap gap-2" x-html="chips('offering', lookups.offerings)"></div>
                    </div>
                    <div>
                        <label class="text-sm font-semibold">{{ __('webapp.form.expects') }} <span class="text-off-black/40 font-normal">({{ __('webapp.common.optional') }})</span></label>
            ...
            <div class="pt-2 flex gap-2">
                <button type="submit" :disabled="busy" class="rounded-xl bg-off-black text-off-white font-semibold px-5 py-3 disabled:opacity-50">
                    <span x-text="busy ? t('form.saving') : (isEdit ? t('form.save_changes') : t('form.save_draft'))"></span>
                </button>
                <a href="{{ $base }}/kolabs" class="rounded-xl bg-off-black/5 font-semibold px-5 py-3">{{ __('webapp.common.cancel') }}</a>
            </div>
            <p class="text-xs text-off-black/50" x-show="!isEdit">{{ __('webapp.form.publish_hint') }}</p>
        </form>
    </main>
</div>
    ...
    function kolabForm() {
        ...
        return {
            ...
            async onCover(e) {
                ...
                else this.error = r.json?.message || t('form.upload_error');
            },
            async init() {
            ...
            async loadExisting() {
                ...
                if (!res.ok) { this.error = t('form.load_error'); return; }
            ...
            async submit() {
                ...
                if (res.status === 422 && res.json?.errors) {
                    ...
                    this.error = res.json?.message || t('form.save_error');
                }
            },
        };
    }
