...
@php
    ...
    $currentIconUrl = old('icon_url', $type->icon_url);
    // Optional minor fallback: a Lucide slug (icon column) for entries with no SVG.
    $currentIcon = old('icon', $type->icon);
    $appliesTo = old('applies_to', $type->applies_to ?? 'both');
@endphp
        ...
        <div class="card"><div class="card-body">
            ...
            @if ($kind === 'business')
                <div class="form-group">
                    <label>Show this category for</label>
                    <select name="applies_to" class="form-control">
                        <option value="both" @selected($appliesTo === 'both')>Both (venue &amp; product / service businesses)</option>
                        <option value="venue" @selected($appliesTo === 'venue')>Venue businesses only</option>
                        <option value="product" @selected($appliesTo === 'product')>Product &amp; service businesses only</option>
                    </select>
                    <small class="text-muted">
                        Controls which onboarding path shows this category pill. The app filters the
                        product path on <code>product</code>/<code>both</code> and the venue path on
                        <code>venue</code>/<code>both</code>.
                    </small>
                </div>
            @endif
